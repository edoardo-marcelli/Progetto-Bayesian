...
We are now ready to compare the performances of these four algorithms. We simulate a Stochastic Volatility process setting arbitrarily the following parameters: $\alpha = -0.1$, $\beta = 0.9$ and $\tau^{2}=0.01$. The resulting observation and state process is shown in the figure below.
...
x     = rep(0,n)
x[1]  = alpha/(1-beta)
y[1]  = rnorm(1,0,exp(x[1]))
for (t in 2:n){
  x[t] = alpha + beta*x[t-1] + rnorm(1,0,tau)
  y[t] = rnorm(1,0,exp(x[t]))
}
alpha.true = alpha
beta.true  = beta
tau2.true  = tau2
...
SVGPFfun<-function(data,N,m0,C0,alpha,beta,tau,r){
  ...
  for(t in 1:length(data)){
    ...
    w1<-w*dnorm(data[t],0,exp(x))*dnorm(x,alpha+beta*xprev,tau)/
      dnorm(x,mean=alpha+beta*xprev+(tau^2)/4*((data[t]^2)*exp(-(alpha+beta*xprev))-2),tau)
...
SVAPFoptfun<-function(data,N,m0,C0,alpha,beta,tau,r){
  ...
  w  = rep(1/N,N)
   ...
   for(t in 1:length(data)){
    ...
    k   = sample(1:N,size=N,replace=FALSE,prob=weight)
    x1   = rnorm(N,alpha+beta*x[k]+(tau^2)/4*((data[t]^2)*exp(-(alpha+beta*x[k]))-2),tau)
    w   <-  w*dnorm(data[t],0,exp(x1))*dnorm(x1,alpha+beta*x[k],tau)/
...
svgpf<-SVGPFfun(y,N,m0,C0,alpha,beta,tau)


svapf<-SVAPFoptfun(y,N,m0,C0,alpha,beta,tau)
...
plotone<-SVFiltplot(dfsv,svpf,"",2,-2)
plottwo<-SVFiltplot(dfsv,svgpf,"",2,-2)
plotthree<-SVFiltplot(dfsv,svapf,"",2,-2)
plotfour<-SVFiltplot(dfsv,svlw,"",2,-2)
...
set.seed(123)
truex<-x
estimatedx<-matrix(NA,ncol=5,nrow=4)
colnames(estimatedx)<-c("N","BPF","GPF", "APF","LWF")
estimatedx[,1]<-c(100,1000,10000,100000)
...
svpf_1<-SVPFfun(y,N,m0,C0, alpha,beta,tau)
svgpf_1<-SVGPFfun(y,N,m0,C0,alpha,beta,tau)
svapf_1<-SVAPFoptfun(y,N,m0,C0,alpha,beta,tau)
svlw_1<-SVLWfun(y,N,m0,C0,ealpha,valpha,ebeta,vbeta,nu,lambda)
