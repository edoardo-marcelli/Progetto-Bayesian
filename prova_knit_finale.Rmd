...
\begin{enumerate}
    ...
    \item Compute: $m^n=a\cdot  \psi^n+(1-a)\cdot \bar \psi$ for $n=1,...,N$, with $a\in (0,1)$ and $h^2=1-a^2$.
...
The Auxiliary Particle Filter with optimal transition kernel uses the same transition kernel of the Guided Particle Filter and we decided to use an auxiliary function $g(x_{t-1})=E(x_{t}|x_{t-1})=\alpha+\beta x_{t-1}$.
...
\item if $ESS<N/2$ then
\begin{enumerate}
\item Draw $I_{k}$ with $P(I_{k}=i) \propto w_{t-1}^{(i)}f_{N}(y_{t}|0,e^{\alpha+\beta x_{t-1}}) $
...
\item Set $\tilde{w}^{(i)}_{t}=w^{(i)}_{t-1}f_{N}(y_{t};0,\alpha+\beta x_{t})$ for $i=1,...,N$
\end{enumerate}
\item Normalize the weights: $w_{t}^{(i)}=\frac{\tilde{w}_{t}^{(i)}}{\sum_{j=1}^{N}(\tilde{w}_{t}^{(j)})}$
\end{enumerate}
\end{itemize}
...
SVAPFoptfun<-function(data,N,m0,C0,alpha,beta,tau,r){
  ...
  for(t in 1:length(data)){
    ...
    for(k in 1:N){
      ...
      w[k]=dnorm(data[t],0,sqrt(exp((x[k]))))/dnorm(data[t],0,sqrt(exp((x_hat))))
    }
    w = w/sum(w)
    ESS  = 1/sum(w^2)
