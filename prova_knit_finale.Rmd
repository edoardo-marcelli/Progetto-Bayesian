...
denote the realizations. Finite sequences are made compact using the semi-colon notation, e.g. $x_{0:t}=(x_0, x_1, \dots, x_t)$ for some $t \geq 0.$ The sets of outcomes are denoted by upper case calligraphic letters, e.g. $\mathcal{X}, \mathcal{Y}.$  A generic $\sigma$-algebra
...
$P_t(x_{t-1},dx_t)$ and $F_t(x_t,dy_t)$, where $t=1,2,\dots$.
\
...
\begin{lemma}\label{kalman-filter}
  Let $X\thicksim\N(m_0,C_0)$ and $Y\mid X\thicksim\N(BX,\Lambda)$, where $C_0$ and $\Lambda$ are positive semi-definite matrices. 
  Then, it holds that
  \begin{align}
...
\begin{align*}
  \P_{t}(X_t\in dx_{t}\mid Y_{1:t}&=y_{1:t})=\N(m_t,C_t), \text{ where} \\ C_t&=R_t[I_{d_x} - B_t'(B_tR_tB_t'+\Lambda_t)^{-1} B_tR_t] \\
...
\begin{equation}
    \frac{1}{N}\sum_{n=1}^Nw(X^n)\xrightarrow{\text{a.s.}} \E_{\M}(w) \label{is_conv_lln}
\end{equation}
...
\end{equation}
Therefore, the IS (auto-normalized) estimator for $\varphi$ is asymptotically normal. Consistency can be proved in an analogous way, applying the strong LLN and lemma \ref{lemma_imp_sam_1}:
\begin{equation}
    \frac{1}{N}\sum_{n=1}^Nw(X^n) \varphi (X^n)\xrightarrow{\text{a.s.}}  \E_\M(\varphi \cdot w)=\E_\Q(\varphi)\cdot \E_\M(w)
\end{equation}
Together with \eqref{is_conv_lln}, we get strong convergence of the estimator (that implies consistency).
\begin{equation}
   \hat \varphi_{AN}=\sum_{n=1}^NW^n \varphi (X^n)\xrightarrow{\text{a.s.}}  \E_\Q(\varphi)
\end{equation}
...
To understand the logic of the previous statement, let us assume that both $\Q_0$ and $\M_0$ admit densities $q_0$, $m_0$ with respect to the same dominating measure $\nu(dx_0)$ and that the transition kernel $M_1$ also admits the density $m_1$. 
...
The \textit{auxiliary particle filter} (APF) constitutes a further extension of the BPF. The GPF allowed to sample from a transition kernel different from the correct ones. Instead, the APF allows to extract the ancestor variables from an arbitrary distribution at the resampling step.\ \
...
\end{equation*}
\item Normalize the auxiliary weights, for $n=1,...,N$:  $\tilde W_t^n:=\frac{\tilde w_t^n}{\sum_{m=1}^N\tilde w_t^m}$.\footnote{It is worth mentioning that the original APF included a final resampling step at the end of each iteration. However, our formulation is more efficient since having two resampling steps might introduce additional Monte Carlo variance. See Johansen and Evers (2007), p. 120.\label{footnote_apf}}
    \end{enumerate}
\end{enumerate}
However, it is worth noting that the optimality results both for the kernel in the GPF and of the auxiliary function in the APF, have  two main weaknesses:
\begin{itemize}
    \item it is not clear that what is optimal at time $t$ remains such at future stages. In other words, it is not truly clear how the minimization of the variance of the inferential weights at each stage affects the overall efficiency of the PF estimators;
...
Firstly, states are drawn from the true transition kernel, while a second resampling step is done at the end of each iteration, according to the original formulation of the algorithm (see footnote \ref{footnote_apf}).
...
We also implement the auxiliary particle filter with optimal transition kernel. Note that we are also using the perfectly adapted auxiliary function and that we resample only once, at the beginning of each iteration. Therefore the algorithm is similar to the one discussed above with the difference that:
...
Let the transition kernel and the likelihood depend on the parameter $\psi$ (i.e., $P_t(x_{t-1},dx_t;\psi)$ and $f_t(y_t|x_t;\psi)$) and let $\pi_0(\cdot)$ be the prior distribution for the parameter $\psi$.\
The LWF can be described by the following algorithm.
\begin{enumerate}
    \item \textbf{Inital stage: }For $n=1,...,N$, at time 0 sample: $X_0^n\thicksim\Pd_0(dx_0)$ and $\psi^n\thicksim \pi_0(\psi)$.
...
\item Normalize the weights, for $n=1,...,N$:  $W_t^n:=\frac{w_t^n}{\sum_{m=1}^Nw_t^m}$.
    \end{enumerate}
\end{enumerate}
At the initial step, $N$ states and $N$ parameter values are sampled independently, respectively, from $\Pd_0$ and $\pi_0$ and the particle weights are set to be constant, as usual.\
...
\begin{equation*}
    \begin{split}
        ...
        Var_{\pi_{t-1}^N} ( \psi)=&Var\big[\E( \psi|A_t^{n})\big]+\E \big[Var( \psi|A_t^{n})\big]=Var\big[\psi^{A_t^{n}}\big]+\E\big[\Omega\big]>\Omega
    \end{split}
\end{equation*}
where by the inequality sign we mean that the matrix $Var\big[\psi^{A_t^{n}}\big]$ is positive definite.
...
\begin{equation}
    \pi^N_{t}(\psi)=\pi^N(\psi|y_{1:t})=\sum_{n=1}^NW_{t}^n\delta_{\psi^n}(d\psi)
\end{equation}
...
\end{align*}
Alternatively, assign them a uniform prior if we have no knowledge of the hyperparameters. Note that in this model we sample from a Gamma distribution (instead of the Normal one, as in the previous description of the LWF) since the parameters of interest are variances, that can take only positive values. Note that we are applying the LWF in the framework of APF with two resampling stage and the correct transition kernel. Next, we describe the algorithm.
...
In this section,\footnote{This section has the purpose to give only a very general idea about convergence of PF estimators.} we cover three propositions that justify the use of the PF algorithms to estimate quantities of interest. Firstly, we present two convergence results for the estimators of the predictive and filtering distributions. As we will discuss, these results hold under reasonable conditions. Then, we will show that the error of the particle estimators are asymptotically distributed according to a Gaussian distribution with known variance. All the convergence results that we present here involve the limit for the number of particles $N$ at each iteration.\
...
These results are of practical use because they provide a way to explicitly compute the asymptotic variance of the PF estimates.

\section{Implementations for Stochastic Volatility Models}\label{Simulated SV data}
...
This part of the analysis is structured as follows: section \ref{Literary review} provides a review of recent research in the stochastic volatility literature, including extensions of the basic specification and the use of particle filters; section \ref{Data description} describes the data which we employ and the transformations that were applied to them. In section \ref{Comparing the filters} we briefly compare the series of filtered states that are obtained from different particle filters; moreover, several PFs are compared in terms of computational cost in approximating the exact filtering distribution implied by our SV model. Section \ref{Realised volatility} employs a non-parametric measure of realised volatility as a proxy for the latent volatility process, allowing a RMSE and MAE comparison between filters which is similar to that of section \ref{Simulated SV data} for synthetic data. In section \ref{Forecast comparison} we conclude by providing a forecast comparison between a SV specification and a model that assumes constant mean and variance for the financial returns, namely a forecasting exercise that would not be possible were it not for particle filtering techniques.

\section{A review of Stochastic Volatility models}\label{Literary review}

In section \ref{Simulated SV data} a simple specification of a Stochastic Volatility model was presented and used as the data generating process of a simulated dataset. Denoting by $y_t$ the log returns at time $t$ and by $x_t$ the latent volatility stochastic process, such state-space model was presented as follows, 
\begin{align*}
...
As regards the series that we will employ in section \ref{Realised volatility}, namely the series of realised volatilities which we will employ as a proxy for the true latent volatilities, the data were retrieved from the Oxford-Man Institute's Realized Library, which provides several different daily non-parametric measures of past volatility.\footnote{Such class of measures will be better presented in section \ref{Realised volatility}, when reviewing the use of realised volatility estimators as proxies for latent volatilities.} We consider a proper rescaling of the \textit{rv5} series, \textit{i.e.} model-free daily volatility estimates based on 5 min intraday return intervals. Also in this case, the chosen interval spanned from June 1st, 2017 to May 30th, 2021. 
...
\section{Comparing the filters}\label{Comparing the filters}
...
\tau^2 & \sim IG(\nu/2,\lambda \nu/2)
\end{align*}
where $\gamma$,$\zeta$,$\pi$,$\phi$,$\nu$ and $\lambda$ are assumed to be known.\footnote{In these empirical applications we keep the values of the parameter we employed in the synthetic data part.}. The initial state is drawn from a normal distribution with commonly used parameters for percentage log returns, as reported by Kastner (2019).\
For simplicity, in $$y_t|x_t \sim N(0,e^{x_t})$$ $$x_t|x_{t-1},\alpha,\beta,\tau^2 \sim N(\alpha+\beta x_{t-1},\tau^2)$$ we set $\alpha=0$, a value that in general, as we argued earlier, also has an empirical justification for percentage log returns. We choose $\beta=0.99$, as one of the basic underlying ideas of SV models is considerable persistence in the volatility. Finally, we set $\tau^2=0.05$.\
In the specification used by the LW filter, we have $\gamma=0$ and $\pi=0.99$, as to center the priors on the point guesses of the simpler model; $\zeta$ and $\phi$ are both set to $0.01$, then allowing a 0.1 standard deviation on both $\alpha$ and $\gamma$. For $\tau^2$, we choose an Inverse Gamma distribution whose expected value coincides with the point guess $0.05$, by setting $\nu=5$ and $\lambda=0.05*3/5$.\

\subsection{Visual Analysis after Filtering}

We now offer a preliminary comparison of the various filtering methods, based on a visual analysis of graphs representing, for each index, the time-varying volatility of the observations, obtained through the application of the different filtering techniques explored in the theoretical analysis. Plots that are compared represent the sequence of percentage log returns for a given index (the observations, in gray) and the sequence of filtered volatilities calculated using filtered states (in red) obtained through a specific filtering technique indicated in the caption of the plot.\footnote{Such filtered volatilities are obtained as follows. For each $t$, let us denote by $\mu_t$ the mean of the generated particles; the filtered volatility is then defined as $e^{\mu_t/2}$. Note that the number of particles was set at 10000 for each filter: the conclusion of the next section will come back to such a choice.} \

```{r echo=FALSE, fig.align='center', fig.cap="Observations and filtered volatility, 3 indices", fig.height=7, fig.pos='H', fig.width=10, message=FALSE, warning=FALSE, results=F}

# Filtered Volatility Plots
...
y1<-realdatasetSPX[,2]
timeframe<-c(1:length(y1))

sv_bpf1<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 1))[,1]
sv_gpfopt1<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 2))[,1]
sv_apf1<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 3))[,1]
sv_lw1<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 4))[,1]
sv_bapf1<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 5))[,1]
sv_sis1<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 6))[,1]
dfsv1<-data.frame(timeframe,y1,sv_bpf1)
dfsv2<-data.frame(timeframe,y1,sv_gpfopt1)
dfsv3<-data.frame(timeframe,y1,sv_apf1)
dfsv4<-data.frame(timeframe,y1,sv_lw1)
dfsv5<-data.frame(timeframe,y1,sv_bapf1)
dfsv6<-data.frame(timeframe,y1,sv_sis1)

Filtplot1<-function(dataframe,title,fun){
  mean=fun
  mean1=exp(mean/2)
  dataframe<-data.frame(dataframe,mean1)
  
  ggplot(dataframe,aes(x=timeframe))+
    geom_line(aes(y=y1, col="Observations", linetype="Observations"))+
    geom_line(aes(y=mean1, col="Filtered Volatility", linetype="Filtered Volatility"))+
    scale_color_manual("",
                       values=c("Observations" = "gray",
                                "Filtered Volatility"= "red"))+
    scale_linetype_manual("",
                          values=c("Observations" = 1,
                                   "Filtered Volatility"=1))+
    labs(x="Time",
         y="")+
    ggtitle(title)+
    theme_bw()+
...
plotobs1<-Filtplot1(dfsv1,"Bootstrap Particle Filter SP500",sv_bpf1)
plotobs2<-Filtplot1(dfsv2,"Opt Ker Guided Particle Filter SP500",sv_gpfopt1)
plotobs3<-Filtplot1(dfsv3,"Auxiliary Particle Filter SP500",sv_apf1)
plotobs4<-Filtplot1(dfsv4,"Liu and West Filter SP500",sv_lw1)
plotobs5<-Filtplot1(dfsv5,"Always Resampling SP500",sv_bapf1)
plotobs6<-Filtplot1(dfsv6,"No Resampling SP500",sv_sis1)

ggarrange(plotobs1,plotobs2)
ggarrange(plotobs3,plotobs4)
ggarrange(plotobs5,plotobs6)

realdatasetSTOXX<-read.xlsx("Dataset 17-21.xlsx", sheet = 2)
y2<-realdatasetSTOXX[,2]
timeframe<-c(1:length(y2))

sv_bpf2<-(read.xlsx("STOXX_filters_10000p.xlsx", sheet = 1))[,1]
sv_gpfopt2<-(read.xlsx("STOXX_filters_10000p.xlsx", sheet = 2))[,1]
sv_apf2<-(read.xlsx("STOXX_filters_10000p.xlsx", sheet = 3))[,1]
sv_lw2<-(read.xlsx("STOXX_filters_10000p.xlsx", sheet = 4))[,1]
sv_bapf2<-(read.xlsx("STOXX_filters_10000p.xlsx", sheet = 5))[,1]
sv_sis2<-(read.xlsx("STOXX_filters_10000p.xlsx", sheet = 6))[,1]
dfsv7<-data.frame(timeframe,y2,sv_bpf2)
dfsv8<-data.frame(timeframe,y2,sv_gpfopt2)
dfsv9<-data.frame(timeframe,y2,sv_apf2)
dfsv10<-data.frame(timeframe,y2,sv_lw2)
dfsv11<-data.frame(timeframe,y2,sv_bapf2)
dfsv12<-data.frame(timeframe,y2,sv_sis2)

Filtplot2<-function(dataframe,title,fun){
  mean=fun
  mean1=exp(mean/2)
  dataframe<-data.frame(dataframe,mean1)
  
  ggplot(dataframe,aes(x=timeframe))+
    geom_line(aes(y=y2, col="Observations", linetype="Observations"))+
    geom_line(aes(y=mean1, col="Filtered Volatility", linetype="Filtered Volatility"))+
    scale_color_manual("",
                       values=c("Observations" = "gray",
                                "Filtered Volatility"= "red"))+
...
plotobs7<-Filtplot2(dfsv7,"Bootstrap Particle Filter STOXX50E",sv_bpf2)
plotobs8<-Filtplot2(dfsv8,"Opt Ker Guided Particle Filter STOXX50E",sv_gpfopt2)
plotobs9<-Filtplot2(dfsv9,"Auxiliary Particle Filter STOXX50E",sv_apf2)
plotobs10<-Filtplot2(dfsv10,"Liu and West Filter STOXX50E",sv_lw2)
plotobs11<-Filtplot2(dfsv11,"Always Resampling STOXX50E",sv_bapf2)
plotobs12<-Filtplot2(dfsv12,"No Resampling STOXX50E",sv_sis2)

ggarrange(plotobs7,plotobs8)
ggarrange(plotobs9,plotobs11)
plotobs10
plotobs12

realdatasetDJI<-read.xlsx("Dataset 17-21.xlsx", sheet = 3)
y3<-realdatasetDJI[,2]
timeframe<-c(1:length(y3))

sv_bpf3<-(read.xlsx("DJI_filters_10000p.xlsx", sheet = 1))[,1]
sv_gpfopt3<-(read.xlsx("DJI_filters_10000p.xlsx", sheet = 2))[,1]
sv_apf3<-(read.xlsx("DJI_filters_10000p.xlsx", sheet = 3))[,1]
sv_lw3<-(read.xlsx("DJI_filters_10000p.xlsx", sheet = 4))[,1]
sv_bapf3<-(read.xlsx("DJI_filters_10000p.xlsx", sheet = 5))[,1]
sv_sis3<-(read.xlsx("DJI_filters_10000p.xlsx", sheet = 6))[,1]
dfsv13<-data.frame(timeframe,y3,sv_bpf3)
dfsv14<-data.frame(timeframe,y3,sv_gpfopt3)
dfsv15<-data.frame(timeframe,y3,sv_apf3)
dfsv16<-data.frame(timeframe,y3,sv_lw3)
dfsv17<-data.frame(timeframe,y3,sv_bapf3)
dfsv18<-data.frame(timeframe,y3,sv_sis3)

Filtplot3<-function(dataframe,title,fun){
  mean=fun
  mean1=exp(mean/2)
  dataframe<-data.frame(dataframe,mean1)
  
  ggplot(dataframe,aes(x=timeframe))+
    geom_line(aes(y=y3, col="Observations", linetype="Observations"))+
    geom_line(aes(y=mean1, col="Filtered Volatility", linetype="Filtered Volatility"))+
    scale_color_manual("",
                       values=c("Observations" = "gray",
                                "Filtered Volatility"= "red"))+
...
plotobs13<-Filtplot3(dfsv13,"Bootstrap Particle Filter DJI",sv_bpf3)
plotobs14<-Filtplot3(dfsv14,"Opt Ker Guided Particle Filter DJI",sv_gpfopt3)
plotobs15<-Filtplot3(dfsv15,"Auxiliary Particle Filter DJI",sv_apf3)
plotobs16<-Filtplot3(dfsv16,"Liu and West Filter DJI",sv_lw3)
plotobs17<-Filtplot3(dfsv17,"Always Resampling DJI",sv_bapf3)
plotobs18<-Filtplot3(dfsv18,"No Resampling DJI",sv_sis3)

ggarrange(plotobs13,plotobs14)
ggarrange(plotobs15,plotobs16)
ggarrange(plotobs17,plotobs18)

```

First, a general point is that the sequences of percentage log returns are different across the three indices. The reasons why this holds can be associated with the index-construction method and the characteristics of the economy and industrial setting it represents. In particular, as previously noticed, there are extensive similarities between the DOW JONES Index and the S&P500, which can be visualized when looking at the percentage of log returns in the corresponding plots, since both are describing the US Economy. However, the STOXX50E differs quite extensively, especially in the different sensitivity to the Covid-19 Crisis and its consequences,  representing the European Economy. Aside from this note, the considerations expressed below on the sequences "filtered volatilities" for the various techniques hold almost uniformly for all the three indices. Therefore, in the next paragraph we will be referring to the set of plots pertaining to one specific index (which could be any of the three).\
...
When focusing the attention on specific filters, first we can discuss the effect of resampling. Indeed, in the plot referring to the Sequential Importance Sampling procedure, the sequence of filtered volatilities is not able to reproduce the peaks detected by all the other filters, often pointing at completely different timing and extents of such peaks. Then, if we instead look at the filtered volatilities obtained with resampling, we can appreciate how the filtered sequences for the Basic,\footnote{By this name, we refer to a particle filter that features no adaptive resampling; rather, resampling is conducted at each step of the algorithm.} Bootstrap, Auxiliary and Optimal Kernel Guided particle filters give rather analogous results, with no differences appearing at a first visual inspection.\
However, the sequence of filtered volatilities obtained by applying the Liu and West filter is somewhat different from the others. Indeed, it shows a "more detailed" sequence of states, meaning that the sequence of filtered volatilities is less smooth and features wider oscillations, with seemingly higher responsiveness to sudden changes in the observations. Therefore, we may expect that, compared to other filtering techniques, it can provide a better representation of the data.
...
Errorcomp1[,2]<-c(0.3017986,0.11645976,0.02906876,0.01511962,0.010462488,0.2342816,0.08340162,0.02145381,0.01110398,0.008013466)
Errorcomp1[,3]<-c(0.3367199,0.09173166,0.03356642,0.01461339,0.009572002,0.2539500,0.06764348,0.02447722,0.01026689,0.007198043)
Errorcomp1[,4]<-c(0.4822315,0.1647978,0.05644653,0.02657352,0.02006171,0.3648792,0.1159503,0.03722911,0.01879724,0.01420517)
Errorcomp1[,5]<-c(0.4561433,0.1740267,0.04786161,0.02741662,0.02085289,0.3643665,0.1352035,0.03833600,0.01939683,0.01481767)
Errorcomp1[,6]<-c(1.566291,1.770554,1.1788696,0.9680993,0.9301619,1.256867,1.396812,0.8683638,0.7212099,0.7182857)
...
As it turns out, such suggestion is confirmed by running the algorithms with N=1000. Passing from 100 to 1000 particles more than halves the RMSEs across the 5 competing filters, with similarly pronounced gains in the MAEs; at N=1000, the Bootstrap and the Guided particle filters still hold their lead over the rest of the field, keeping a shrinking hedge over the Auxiliary PF, which shows most impressive gains when passing from 100 to 1000 particles. Further sizable gains for every filter are observed when increasing the number of particles to 5000, a level at which both the RMSE and the MAE agree on a slight hedge of the Guided and the Bootstrap filters over the Auxiliary and the Basic PFs.\
Nevertheless, we would argue that already for N=5000, but even more clearly for N=10000, the difference between the Bootstrap, the Guided and the Auxiliary particle filters is so small that none of these filters can be straightforwardly chosen over the others, a figure which visual inspection of the graphs below can confirm. Interestingly enough, no gains seem to arise from resampling at each step with respect to adaptive resampling (note that the Basic PF is preferred to the APF only twice, by the RMSE), whereas the need of some degree of resampling is apparent, as shown by the awful performance of the SIS algorithm with any number of particles. Moreover, the RMSE and the MAE show infrequent disagreement, delivering a rather clear picture of the comparison.

```{r echo=FALSE, fig.align='center', fig.cap="Approximations at N=10000, Bootstrap benchmark", fig.height=4, fig.pos='H', fig.width=8, message=FALSE, warning=FALSE, results=F}
...
realdatasetSPX<-read.xlsx("Dataset 17-21.xlsx", sheet = 1)
y<-realdatasetSPX[,2]
btbenchmark<-(read.xlsx("svbpf50000p.xlsx", sheet = 1))[,1]
timeframe<-c(1:length(y))
...
sv_bapf1<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 5))[,1]
sv_sis1<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 6))[,1]
sv_bpf1sd<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 1))[,2]
sv_gpfopt1sd<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 2))[,2]
sv_apf1sd<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 3))[,2]
sv_bapf1sd<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 5))[,2]
sv_sis1sd<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 6))[,2]

dfsv1<-data.frame(timeframe,y,btbenchmark,sv_bpf1,sv_bpf1sd)
dfsv2<-data.frame(timeframe,y,btbenchmark,sv_gpfopt1,sv_gpfopt1sd)
dfsv3<-data.frame(timeframe,y,btbenchmark,sv_apf1,sv_apf1sd)
dfsv5<-data.frame(timeframe,y,btbenchmark,sv_bapf1,sv_bapf1sd)
dfsv6<-data.frame(timeframe,y,btbenchmark,sv_sis1,sv_sis1sd)

Filtplot4<-function(dataframe,title,funmean,funsd){
  mean=funmean
  sd=funsd
  dataframe<-data.frame(dataframe,mean)
  
  ggplot(dataframe,aes(x=timeframe))+
    geom_line(aes(y=btbenchmark, col="BT benchmark", linetype="BT benchmark"))+
    geom_line(aes(y=mean, col="Filtered States", linetype="Filtered States"))+
    geom_ribbon(aes(ymin = mean -1.96*sd, ymax = mean +1.96*sd),
                fill="red",alpha=0.16) +
    scale_color_manual("",
                       values=c("BT benchmark" = "black",
                                "Filtered States"= "red"))+
    scale_linetype_manual("",
                          values=c("BT benchmark" = 1,
                                   "Filtered States"=1))+
    labs(x="Time",
         y="")+
    ggtitle(title)+
...
plotcomp1<-Filtplot4(dfsv1,"Bootstrap Particle Filter SP500",sv_bpf1,sv_bpf1sd)
plotcomp2<-Filtplot4(dfsv2,"Opt Ker Guided Particle Filter SP500",sv_gpfopt1,sv_gpfopt1sd)
plotcomp3<-Filtplot4(dfsv3,"Auxiliary Particle Filter SP500",sv_apf1,sv_apf1sd)
plotcomp5<-Filtplot4(dfsv5,"Always Resampling SP500",sv_bapf1,sv_bapf1sd)
plotcomp6<-Filtplot4(dfsv6,"No Resampling SP500",sv_sis1,sv_sis1sd)
...
Errorcomp2[,2]<-c(0.2998373,0.11719984,0.03339627,0.02213888,0.01879835,0.2334205,0.08306614,0.02414570,0.01444042,0.01184698)
Errorcomp2[,3]<-c(0.3365006,0.09527023,0.03979288,0.02073930,0.01744916,0.2540961,0.06926543,0.02793461,0.01360319,0.01149824)
Errorcomp2[,4]<-c(0.4828496,0.1665464,0.05696147,0.03056349,0.01929485,0.3651631,0.1174350,0.03692383,0.02031451,0.01437991)
Errorcomp2[,5]<-c(0.4555563,0.1715253,0.04847040,0.02916413,0.02211777,0.3634416,0.1343189,0.03877838,0.02075878,0.01495394)
Errorcomp2[,6]<-c(1.565179,1.770452,1.1782202,0.9681921,0.9298538,1.254832,1.397196,0.8666459,0.7203661,0.7181958)
...
Using the APF as the benchmark does not deliver a different picture than the one we previously had. Leaving the SIS algorithm aside (whose analogous performance comes as no surprise), it is interesting to note how once again the Auxiliary particle filter actually fares worse than the Bootstrap and the Guided PFs at each threshold (though very slightly so for the higher ones). Its performance ameliorates dramatically at N=1000, while it still shows some sizable improvements even when passing from N=5000 to N=10000, almost halving its errors. Moreover, note that the mere fact that at N=10000 the Guided and the Bootstrap particle filters approximate better the APF benchmark suggests that the APF still has some margins for relative improvement even at high levels of N, pointing at higher computational costs than the competing filters: trivially, since at N=50000 its RMS and MA difference with respect to this benchmark must be zero, the APF overtakes the Guided and the Bootstrap PFs at N $>>10000$.\
...
Errorcomp3[1,]<-c(0.015181843,0.008679359)
Errorcomp3[2,]<-c(0.013735482,0.006467285) #updated
```
...
By comparing the two benchmark approximations with N=50000, we can see that some discrepancy between competing approximations still persists at high numbers of particles. Conveniently - as a different outcome would point at a theoretically-unsound difference between the limiting distributions of the filters - running the Auxiliary and the Bootstrap filters with 100000 particles testifies a shrinking difference between the approximations - at increasingly large computational costs, though.\

Overall, we should not overplay tiny differences.\footnote{Indeed, the mere extent of such difference also depends on the real dataset which is used for the comparison, implying different target distributions. For example, running the same experiment using the STOXX50E dataset, we have similar differences between the filters at N=50000, with RMS difference between the Bootstrap and the Auxiliary equal to 0.016121989 and mean absolute difference equal to 0.009306925. The extent of the difference clearly depends on the chosen parameters too: for example, at N=50000, choosing $\alpha=-0.0031$, $\beta=0.9951$ and $\tau^2=0.0074$, using SP500 data yields RMS difference equal to 0.04408 and MA difference equal to 0.01668, while using STOXX50E data we have 0.01236279 RMS difference and 0.008028818 mean absolute difference.} Rather, we prefer to stress the points that emerged from both benchmark analyses and we can therefore support quite confidently.\
...
In order to evaluate the different filters according to the evaluation tool represented by the realized volatility, we start once more from the visual inspection of the plots. For any of the three indices, below graphs are shown including both the filtered volatility obtained from the filtered states (for any of the Sequential MC methods described in the theoretical section) and the corresponding rescaled realised volatility series.\footnote{For simplicity,troughout the rest of this section we will omit "rescaled" when refering to the rescaled realised volatility.} As compared to the visual analysis we conducted in section \ref{Comparing the filters}, we can now use such a proxy to determine the relative performance of the competing filters.\

Once more, most algorithms used (aside from SIS and Liu-West) seem to provide indistinguishable results. The SIS algorithm appears completely inadequate to describe the "true" volatility process: as we argued in section \ref{pf_intro}, the diverging variance of its weights on the particles prevents it from delivering a good enough approximation of the target distribution. On top of that, it does not seem the case that the SIS algorithm, by falling short of the SV target distribution, ends up fitting the proxy better.\footnote{As we will now see, this seems instead to be the case for our preferred algorithms, namely those adopting adaptive resampling rules: a better approximation of the SV target filtering distribution yields slightly higher RMSEs and MAEs than those resulting from less accurate MC approximations.}\
...
```{r echo=FALSE, fig.align='center', fig.cap="Filtered volatility and realised volatility", fig.height=4, fig.pos='H', fig.width=8, message=FALSE, warning=FALSE, results=F}
...
sv_bapf1<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 5))[,1]
sv_sis1<-(read.xlsx("SP500_filters_10000p.xlsx", sheet = 6))[,1]
dfsv1<-data.frame(timeframe,y,realisedx1,sv_bpf1)
dfsv2<-data.frame(timeframe,y,realisedx1,sv_gpfopt1)
dfsv3<-data.frame(timeframe,y,realisedx1,sv_apf1)
dfsv4<-data.frame(timeframe,y,realisedx1,sv_lw1)
dfsv5<-data.frame(timeframe,y,realisedx1,sv_bapf1)
dfsv6<-data.frame(timeframe,y,realisedx1,sv_sis1)

Filtplot5<-function(dataframe,title,fun,realisedx){
  mean=fun
  mean1=exp(mean/2)
  dataframe<-data.frame(dataframe,mean1)
  
  ggplot(dataframe,aes(x=timeframe))+
    geom_line(aes(y=realisedx, col="Proxy", linetype="Proxy"))+
    geom_line(aes(y=mean1, col="Filtered Volatility", linetype="Filtered Volatility"))+
    scale_color_manual("",
                       values=c("Proxy" = "black",
                                "Filtered Volatility"= "red"))+
    scale_linetype_manual("",
                          values=c("Proxy" = 3,
                                   "Filtered Volatility"=1))+
...
plotproxy1<-Filtplot5(dfsv1,"Bootstrap Particle Filter SP500",sv_bpf1,realisedx1)
plotproxy2<-Filtplot5(dfsv2,"Opt Ker Guided Particle Filter SP500",sv_gpfopt1,realisedx1)
plotproxy3<-Filtplot5(dfsv3,"Auxiliary Particle Filter SP500",sv_apf1,realisedx1)
plotproxy4<-Filtplot5(dfsv4,"Liu and West Filter SP500",sv_lw1,realisedx1)
plotproxy5<-Filtplot5(dfsv5,"Always Resampling SP500",sv_bapf1,realisedx1)
plotproxy6<-Filtplot5(dfsv6,"No Resampling SP500",sv_sis1,realisedx1)
...
sv_bapf2<-(read.xlsx("STOXX_filters_10000p.xlsx", sheet = 5))[,1]
sv_sis2<-(read.xlsx("STOXX_filters_10000p.xlsx", sheet = 6))[,1]
dfsv7<-data.frame(timeframe,y,realisedx2,sv_bpf2)
dfsv8<-data.frame(timeframe,y,realisedx2,sv_gpfopt2)
dfsv9<-data.frame(timeframe,y,realisedx2,sv_apf2)
dfsv10<-data.frame(timeframe,y,realisedx2,sv_lw2)
dfsv11<-data.frame(timeframe,y,realisedx2,sv_bapf2)
dfsv12<-data.frame(timeframe,y,realisedx2,sv_sis2)

plotproxy7<-Filtplot5(dfsv7,"Bootstrap Particle Filter STOXX",sv_bpf2,realisedx2)
plotproxy8<-Filtplot5(dfsv8,"Opt Ker Guided Particle Filter STOXX",sv_gpfopt2,realisedx2)
plotproxy9<-Filtplot5(dfsv9,"Auxiliary Particle Filter STOXX",sv_apf2,realisedx2)
plotproxy10<-Filtplot5(dfsv10,"Liu and West Filter STOXX",sv_lw2,realisedx2)
plotproxy11<-Filtplot5(dfsv11,"Always Resampling STOXX",sv_bapf2,realisedx2)
plotproxy12<-Filtplot5(dfsv12,"No Resampling STOXX",sv_sis2,realisedx2)
...
The above intuitions are confirmed by looking at the table below, where the RMSE and MAE are shown for each filter (run with a number of particles equal to 10000), considering the realised volatility as benchmark. Indeed, for any index, most of the filters give rather similar results in terms of RMSE and MAE. For any index and for any measure, the worst performing is the SIS algorithm, while the best performing is the Liu and West particle filter. Although the latter algorithm was outperformed in the analysis we conducted on synthetic data, in fact we expected such reversal of fortunes: intuitively, while the rest of the field is built on a model with fixed guesses for the unknown parameters, the Liu and West filters enjoys more flexibility thanks to the higher variability implied by its additional hierarchical level and by its parameter learning, two features that should both pay off in real data analyses.\
...
comparablevol<-function(fun){exp(fun/2)}
Errorcomp3[1,2]<-RMSE(realisedx1,comparablevol(sv_bpf1))
Errorcomp3[1,3]<-RMSE(realisedx1,comparablevol(sv_gpfopt1))
Errorcomp3[1,4]<-RMSE(realisedx1,comparablevol(sv_apf1))
Errorcomp3[1,5]<-RMSE(realisedx1,comparablevol(sv_bapf1))
Errorcomp3[1,6]<-RMSE(realisedx1,comparablevol(sv_lw1))
Errorcomp3[1,7]<-RMSE(realisedx1,comparablevol(sv_sis1))
Errorcomp3[2,2]<-MAE(realisedx1,comparablevol(sv_bpf1))
Errorcomp3[2,3]<-MAE(realisedx1,comparablevol(sv_gpfopt1))
Errorcomp3[2,4]<-MAE(realisedx1,comparablevol(sv_apf1))
Errorcomp3[2,5]<-MAE(realisedx1,comparablevol(sv_bapf1))
Errorcomp3[2,6]<-MAE(realisedx1,comparablevol(sv_lw1))
Errorcomp3[2,7]<-MAE(realisedx1,comparablevol(sv_sis1))

volatilitySTOXX<-read.xlsx("Dataset 17-21.xlsx", sheet = 2)
x2<-volatilitySTOXX[,3]
realisedx2<-x2
sv_bpf2<-(read.xlsx("STOXX_filters_10000p.xlsx", sheet = 1))[,1]
sv_gpfopt2<-(read.xlsx("STOXX_filters_10000p.xlsx", sheet = 2))[,1]
sv_apf2<-(read.xlsx("STOXX_filters_10000p.xlsx", sheet = 3))[,1]
...
MAE<-function(x,xhat){mean(abs(x-xhat))}
comparablevol<-function(fun){exp(fun/2)}
Errorcomp3[3,2]<-RMSE(realisedx2,comparablevol(sv_bpf2))
Errorcomp3[3,3]<-RMSE(realisedx2,comparablevol(sv_gpfopt2))
Errorcomp3[3,4]<-RMSE(realisedx2,comparablevol(sv_apf2))
Errorcomp3[3,5]<-RMSE(realisedx2,comparablevol(sv_bapf2))
Errorcomp3[3,6]<-RMSE(realisedx2,comparablevol(sv_lw2))
Errorcomp3[3,7]<-RMSE(realisedx2,comparablevol(sv_sis2))
Errorcomp3[4,2]<-MAE(realisedx2,comparablevol(sv_bpf2))
Errorcomp3[4,3]<-MAE(realisedx2,comparablevol(sv_gpfopt2))
Errorcomp3[4,4]<-MAE(realisedx2,comparablevol(sv_apf2))
Errorcomp3[4,5]<-MAE(realisedx2,comparablevol(sv_bapf2))
Errorcomp3[4,6]<-MAE(realisedx2,comparablevol(sv_lw2))
Errorcomp3[4,7]<-MAE(realisedx2,comparablevol(sv_sis2))

volatilityDJI<-read.xlsx("Dataset 17-21.xlsx", sheet = 3)
x3<-volatilityDJI[,3]
realisedx3<-x3
sv_bpf3<-(read.xlsx("DJI_filters_10000p.xlsx", sheet = 1))[,1]
sv_gpfopt3<-(read.xlsx("DJI_filters_10000p.xlsx", sheet = 2))[,1]
...
MAE<-function(x,xhat){mean(abs(x-xhat))}
comparablevol<-function(fun){exp(fun/2)}
Errorcomp3[5,2]<-RMSE(realisedx3,comparablevol(sv_bpf3))
Errorcomp3[5,3]<-RMSE(realisedx3,comparablevol(sv_gpfopt3))
Errorcomp3[5,4]<-RMSE(realisedx3,comparablevol(sv_apf3))
Errorcomp3[5,5]<-RMSE(realisedx3,comparablevol(sv_bapf3))
Errorcomp3[5,6]<-RMSE(realisedx3,comparablevol(sv_lw3))
Errorcomp3[5,7]<-RMSE(realisedx3,comparablevol(sv_sis3))
Errorcomp3[6,2]<-MAE(realisedx3,comparablevol(sv_bpf3))
Errorcomp3[6,3]<-MAE(realisedx3,comparablevol(sv_gpfopt3))
Errorcomp3[6,4]<-MAE(realisedx3,comparablevol(sv_apf3))
Errorcomp3[6,5]<-MAE(realisedx3,comparablevol(sv_bapf3))
Errorcomp3[6,6]<-MAE(realisedx3,comparablevol(sv_lw3))
Errorcomp3[6,7]<-MAE(realisedx3,comparablevol(sv_sis3))
...
Finally, we can see from the table below the RMSE and MAE obtained with a number of particles equal to 50000, for the SP500 and the EuroStoxx50 indices. We consider the Bootstrap particle filter and the Auxiliary particle filter, which we employed in section \ref{Comparing the filters}; in addition to these, we add the Liu and West filter, which did not feature in that section of this work.\
In the case of the SP500 index, although the number of particles increases, both RMSE and MAE increase (although by very small amounts) for the Auxiliary Particle Filter, while they slightly decrease for the Bootstrap PF. Instead, for the Liu and West filter we see a sizable deterioration in the performance: this suggests that the parameter learning process embedded in the LWF is actually pointing in a different direction than that of the proxy, suggesting that either the proxy does not capture well the latent volatility, or a SV model may not be the best tool to disclose such latent process.\footnote{Note that at N=1000, the LWF yields a 0.3794359 RMSE and a 0.2687336 MAE. In words, worse approximations of the target distribution of the LWF are actually best at replicating the proxy for the SP500. A possible interpretation goes, once again, as that reported in the main text.}\
In the case of the EuroStoxx50 index, while the Bootstrap filter slightly increases both RMSE and MAE when N is raised to 50000, the APF shows mild improvements in its MAE performance, while the RMSE criterion points in the opposite direction. The criteria disagree also in the case of the LW filter, as the MAE mildly prefers the performance of N=10000, while the RMSE detects minor improvements for N=50000.
...
x1<-volatilitySPX[,3]
realisedx1<-x1
sv_bpf1_5<-(read.xlsx("svbpf50000p.xlsx", sheet = 1))[,1]
sv_apf1_5<-(read.xlsx("svapf50000p.xlsx", sheet = 1))[,1]
sv_lw1_5<-(read.xlsx("svlw50000p.xlsx", sheet = 1))[,1]
RMSE<-function(x,xhat){sqrt(mean((x-xhat)^2))}
MAE<-function(x,xhat){mean(abs(x-xhat))}
comparablevol<-function(fun){exp(fun/2)}
Errorcomp4[1,2]<-RMSE(realisedx1,comparablevol(sv_bpf1_5))
Errorcomp4[1,3]<-RMSE(realisedx1,comparablevol(sv_apf1_5))
Errorcomp4[1,4]<-RMSE(realisedx1,comparablevol(sv_lw1_5))
Errorcomp4[2,2]<-MAE(realisedx1,comparablevol(sv_bpf1_5))
Errorcomp4[2,3]<-MAE(realisedx1,comparablevol(sv_apf1_5))
Errorcomp4[2,4]<-MAE(realisedx1,comparablevol(sv_lw1_5))

volatilitySPX<-read.xlsx("Dataset 17-21.xlsx", sheet = 2)
x2<-volatilitySPX[,3]
realisedx2<-x2
sv_bpf2_5<-(read.xlsx("svbpf50000p.xlsx", sheet = 2))[,1]
sv_apf2_5<-(read.xlsx("svapf50000p.xlsx", sheet = 2))[,1]
sv_lw2_5<-(read.xlsx("svlw50000p.xlsx", sheet = 2))[,1]
RMSE<-function(x,xhat){sqrt(mean((x-xhat)^2))}
MAE<-function(x,xhat){mean(abs(x-xhat))}
comparablevol<-function(fun){exp(fun/2)}
Errorcomp4[3,2]<-RMSE(realisedx2,comparablevol(sv_bpf2_5))
Errorcomp4[3,3]<-RMSE(realisedx2,comparablevol(sv_apf2_5))
Errorcomp4[3,4]<-RMSE(realisedx2,comparablevol(sv_lw2_5))
Errorcomp4[4,2]<-MAE(realisedx2,comparablevol(sv_bpf2_5))
Errorcomp4[4,3]<-MAE(realisedx2,comparablevol(sv_apf2_5))
Errorcomp4[4,4]<-MAE(realisedx2,comparablevol(sv_lw2_5))
...
realisedx<-x
ErrorvolSPX<-matrix(NA,ncol=7,nrow=2)
colnames(ErrorvolSPX)<-c("N","BPF", "GPFOPT", "APF","BAPF","LWF", "SIS")
rownames(ErrorvolSPX)<-c("RMSE SP500","MAE SP500")
ErrorvolSPX[,1]<-c(1000)
RMSE<-function(x,xhat){sqrt(mean((x-xhat)^2))}
MAE<-function(x,xhat){mean(abs(x-xhat))}
comparablevol<-function(fun){exp((Filtervalues(fun)$mean)/2)}
ErrorvolSPX[1,2]<-RMSE(realisedx,comparablevol(svbpf))
ErrorvolSPX[1,3]<-RMSE(realisedx,comparablevol(svgpfopt))
ErrorvolSPX[1,4]<-RMSE(realisedx,comparablevol(svapf))
ErrorvolSPX[1,5]<-RMSE(realisedx,comparablevol(svbapf))
ErrorvolSPX[1,6]<-RMSE(realisedx,comparablevol(svlw))
ErrorvolSPX[1,7]<-RMSE(realisedx,comparablevol(svsis))
ErrorvolSPX[2,2]<-MAE(realisedx,comparablevol(svbpf))
ErrorvolSPX[2,3]<-MAE(realisedx,comparablevol(svgpfopt))
ErrorvolSPX[2,4]<-MAE(realisedx,comparablevol(svapf))
ErrorvolSPX[2,5]<-MAE(realisedx,comparablevol(svbapf))
ErrorvolSPX[2,6]<-MAE(realisedx,comparablevol(svlw))
ErrorvolSPX[2,7]<-MAE(realisedx,comparablevol(svsis))
```
```{r echo=FALSE, message=FALSE, warning=FALSE, results="asis"}
library(kableExtra)
kbl(ErrorvolSPX, align="c", longtable = T, booktabs = T, caption="RMSE and MAE, realised volatility as benchmark (SPX)",digits=5)
```
}\
...
\section{Comparing Constant and Stochastic Volatility Models of Equity Returns Using the BPF}\label{Forecast comparison}

In this section, we round up the appplications chapter using the particle filter in a simple model-comparison
exercise, underlining the importance of taking stochastic
volatility into account in modeling equity returns. We follow Johannes, Korteweg and Polson (2014), comparing density forecasts through relative predictive and cumulative likelihoods.

We analyze the previously mentioned dataset of log close-to-close returns of the S&P500, from 2017 to 2021. Consider a stochastic volatility model
$$
...
with weighted particles obtained with a Bootstrap Particle Filter.

Figure \ref{fig:cll_plot} reports the time series of the cumulative log-likelihood
...
```{r cll_plot, echo=FALSE, fig.align='center', fig.cap="Relative log-likelihood of SP500 daily log returns, CV and SV", fig.height=3, fig.pos='H', fig.width=10, results=F}
...
SVAPFoptfun<-function(data,N,m0,C0,alpha,beta,tau,r){
  ...
  for(t in 1:length(data)){
    ...
    if(ESS<(N/r)){
      epsilon=alpha+beta*x+((tau^2)/4)*((data[t]^2)*exp(-alpha-beta*x)-2)
      weight = w*exp((1/(2*tau^2))*((epsilon)^2-(alpha+beta*x)^2)-((data[t]^2)/2)*
      exp(-alpha-beta*x)*(1+alpha+beta*x))
      k   = sample(1:N,size=N,replace=TRUE,prob=weight)
      x1   = rnorm(N,alpha+beta*x[k]+(tau^2)/4*((data[t]^2)*exp(-(alpha+beta*x[k]))-2),tau)
      w   = rep(1/N,N)
    }else{
