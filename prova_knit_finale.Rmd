...
Fortunately, convergence results about particle filters ensure that when the number of particles diverges, the approximation converges to the target distribution.\footnote{Riferimento alla sezione di convergenza?.} Thus, we run one of the particle filters with a very large number of particles, and assume that such approximation is close enough to the target to be used itself as the distribution the filters should tend to.\
...
\section{Comparing Constant and Stochastic Volatility Models of Equity Returns}\label{Forecast comparison}
