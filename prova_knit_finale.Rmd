...
\begin{lemma}\label{kalman-filter}
  Let $X\thicksim\N(m_0,C_0)$ and $Y\mid X\thicksim\N(BX,R)$, where $R$ and $Q_0$ are positive semi-definite matrices. 
  Then, it holds that
  \begin{align}
    X\mid Y=y&\thicksim\N(m_1,C_1),\\
    C_1&= C_0 (I_{d_x} - B'(B C_0 B'+ R)^{-1} B C_0) \\
    m_1&=[I_{d_x} - C_0 B'(B C_0 B'+ R)^{-1} B ]m_0 + C_0 B'(B C_0 B'+ R)^{-1}y.
  \end{align}
\end{lemma}
\begin{proof*}
...
Now, by Bayes' Theorem, 
    ...
    \begin{align*}
        p(x\mid Y=y)  &\propto p(x) p(y\mid x) \\
                    &\propto \text{exp}\Big(-\frac{1}{2}X' C_0^{-1}X + X' C_0^{-1}m_0-\frac{1}{2}(y-BX)'R^{-1}(y-BX)) \\
                    &\propto \text{exp}(-\frac{1}{2}X' C_0^{-1}X + X' C_0^{-1}m_0-\frac{1}{2}X' B' R^{-1}BX+X' B' R^{-1}y)\\
                    &=\text{exp}(-\frac{1}{2}X'(C_0^{-1}+B' R^{-1}B)X+
                    X'(C_0^{-1}m_0+B' R^{-1}y)\Big).
    \end{align*}

Hence, it follows that 
\begin{equation*}
    C_1=Var(X\mid Y=y)= (C_0^{-1}+ B' R^{-1} B)^{-1}=C_0-C_0 B'(B C_0 B'+R)^{-1}B C_0
\end{equation*}
and 
    \begin{align*}
        m_1 &=\E(X\mid Y=y) = C_1^{-1}(C_0^{-1}m_0+B' R^{-1}y)\\
            &= (C_0-C_0B'(B C_0B'+R)^{-1}B C_0)(C_0^{-1}m_0+B' R^{-1}y)\\
            &= m_0-C_0B'(B C_0B'+R)^{-1}B m_0 + C_0 B' R^{-1}y - C_0B'(B C_0B'+R)^{-1}B C_0 B' R^{-1}y \\
            &=[I_{d_x} - C_0 B'(B C_0 B'+ R)^{-1} B ]m_0 + C_0 B'(B C_0 B'+ R)^{-1}y.
    \end{align*}
\end{proof*}
...
\end{equation}
Letting $E_T=A_tC_{t-1}A_t'+\Sigma_t,$ and applying lemma \ref{kalman-filter}, it follows that the filtering distribution is 
\begin{align*}
  \P_{t}(X_t\in dx_{t}\mid Y_{1:t-1}&=y_{1:t-1})=\N(m_t,C_t), \text{where} \\ C_t&=E_t[I_{d_x} - B_t'(B_tE_tB_t'+R_t)^{-1} B_tE_t] \\
