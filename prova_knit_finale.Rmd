...
\item Set $\tilde{w}^{(i)}_{t}=w^{(i)}_{t-1}g(x_{t-1})$ for $i=1,...,N$
\end{enumerate}
\item Normalize the weights: $w_{t}^{(i)}=\frac{\tilde{w}_{t}^{(i)}}{\sum_{j=1}^{N}(\tilde{w}_{t}^{(j)})}$
\end{enumerate}
\end{itemize}
...
svapf<-SVAPFoptfun(y,N,m0,C0,alpha,beta=0,tau)
...
We provide both a graphical and a numerical comparison and 
...
plotone<-SVFiltplot(dfsv,svpf,"",1.5,-2.5)
plottwo<-SVFiltplot(dfsv,svgpf,"",1.5,-2.5)
plotthree<-SVFiltplot(dfsv,svapf,"",1.5,-2.5)
plotfour<-SVFiltplot(dfsv,svlw,"",1.5,-2.5)
...
svapf_1<-SVAPFoptfun(y,N,m0,C0,alpha,beta=0,tau)
